...
The `TransferCommand.description` field encodes a JSON object
`{ paymentId, resourceUrl }` where `paymentId` is a UUID generated by the
client at payment time (not transmitted in the header payload). The facilitator
validates both fields in Rule 9.
