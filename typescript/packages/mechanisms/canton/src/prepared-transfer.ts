...
/** The money-bearing terms of an `AmuletRules_CreateTransferPreapproval`
 *  choice argument. `expiresAtMicros` is the raw Daml `Time` (µs since epoch),
 *  left unconverted so the comparison happens against the signed value. */
export interface ExtractedSelfPreapproval {
  receiver: string;
  provider: string;
...
/**
 * Extract receiver / provider / expiresAt from an
 * `AmuletRules_CreateTransferPreapproval` choice argument.
 *
 * MEASURED, not guessed. The declaration order and leaf types below were read
 * off a real MainNet prepared transaction captured from the live relay
 * (`src/__fixtures__/mainnet-self-preapproval-create.b64`), because every time
 * this repo assumed a wire shape from a hand-built fixture it was wrong in a way
 * the fixture hid — the zigzag nonce, the fixed64 timestamp, the Optional DSO.
 * Order: [0] context [1] inputs [2] receiver:Party [3] provider:Party
 * [4] expiresAt:Time [5] expectedDso:Optional Party.
 *
 * `expectedDso` is deliberately not returned: the choice validates it itself, so
 * a relay lying there gets a failed transaction rather than a payday. `receiver`
 * and `provider` have no such self-check, which is why they are the point.
 *
 * @param chosenValue
 */
export function extractSelfPreapproval(chosenValue: Uint8Array): ExtractedSelfPreapproval {
  const entries = recordEntries(chosenValue);
  const receiverE = entryByDeclOrder(entries, 2, "receiver");
  const providerE = entryByDeclOrder(entries, 3, "provider");
  const expiresE = entryByDeclOrder(entries, 4, "expiresAt");
  const receiver = receiverE ? leafOf(receiverE.value) : undefined;
  const provider = providerE ? leafOf(providerE.value) : undefined;
  const expires = expiresE ? leafOf(expiresE.value) : undefined;
  if (!receiver || receiver.kind !== "party") {
    throw new PreparedDecodeError(
      "self-preapproval argument: receiver is not a party leaf at its declaration position",
    );
  }
  if (!provider || provider.kind !== "party") {
    throw new PreparedDecodeError(
      "self-preapproval argument: provider is not a party leaf at its declaration position",
    );
  }
  if (!expires || expires.kind !== "timestamp") {
    throw new PreparedDecodeError(
      "self-preapproval argument: expiresAt is not a timestamp leaf at its declaration position",
    );
  }
  return {
    receiver: receiver.value,
    provider: provider.value,
    expiresAtMicros: expires.value,
  };
}
...
/** The registry analogue of `LockedAmulet_UnlockV2` for the CLAIM path:
 *  accepting a DA-registry two-step offer exercises `TransferRule_TwoStepTransfer`
 *  on the registry's TransferRule, which consumes the offer and creates the
 *  receiver's Holding. MEASURED on a live MainNet USDCx offer-accept prepared
 *  by the participant (agent-wallet/src/__fixtures__/mainnet-usdcx-registry-
 *  accept.b64): root TransferInstruction_Accept → TransferRule_TwoStepTransfer
 *  + Archive + one Holding create. Template-pinned exactly like
 *  REGISTRY_UTILITY_DIRECT_TRANSFER, its outbound twin, so a look-alike choice
 *  on some other template still fails closed. Byte-identical for Amulet, whose
 *  accept tree never carries this choice. */
const REGISTRY_UTILITY_TWO_STEP_ACCEPT = {
  choiceId: "TransferRule_TwoStepTransfer",
  templates: ["Utility.Registry.V0.Rule.Transfer:TransferRule"],
} as const;

const ACCEPT_CONSEQUENCE_CHOICES = [
  "Archive",
  "LockedAmulet_UnlockV2",
  EVENT_LOG_HOLDINGS_CHANGE,
] as const;

/** The accept whitelist for a REGISTRY claim. `TransferRule_TwoStepTransfer`
 *  is value-moving (it archives the sender's holding and creates the
 *  receiver's), so it is admitted only when the caller says it is claiming a
 *  registry token (`expect.instrumentAdmin`), and then the tree is held to the
 *  registry accept SHAPE (see assertPreparedAcceptMatches): root on the
 *  registrar's `TransferOffer` template, every create a registry `Holding`
 *  that names the agent and no party outside {agent, registrar, trusted
 *  registry parties}. A Canton Coin claim never consults this list, so an
 *  Amulet tree carrying the registry node is refused exactly as before. */
const REGISTRY_ACCEPT_CONSEQUENCE_CHOICES = [
  ...ACCEPT_CONSEQUENCE_CHOICES,
  REGISTRY_UTILITY_TWO_STEP_ACCEPT,
] as const;
const REGISTRY_TRANSFER_OFFER_TEMPLATE = "Utility.Registry.App.V0.Model.Transfer:TransferOffer";

/** Caller-intent expectation for a `TransferInstruction_Accept` (claim) prepare. */
export interface PreparedAcceptExpectation {
  selfParty: string;
  nowMs?: number;
  /** Registry token being claimed — the instrument admin the /pending row
   *  named. Present ONLY for a registry token; a Canton Coin claim leaves it
   *  undefined and the registry two-step accept rule is never consulted. */
  instrumentAdmin?: string;
  /** Independently-trusted registry parties (operator/bridge) that may appear
   *  in a created holding next to the agent and the registrar. Anything else
   *  in a created holding is a redirected recipient → refused. */
  trustedRegistryParties?: ReadonlySet<string>;
}
...
export function assertPreparedAcceptMatches(
  ...
  const registryIntent = expect.instrumentAdmin !== undefined;
  const { exercise: root } = assertSingleAllowedRootExercise(
    decoded,
    ACCEPT_CHOICE,
    ACCEPT_CHOICE,
    registryIntent ? REGISTRY_ACCEPT_CONSEQUENCE_CHOICES : ACCEPT_CONSEQUENCE_CHOICES,
  );

  // Registry accept shape (see REGISTRY_ACCEPT_CONSEQUENCE_CHOICES). The
  // accept arm pins no amount and no receiver — the agent is accepting what
  // the offer says — so the DIRECTION of value is proven structurally instead:
  // the root must be the registrar's own TransferOffer (an interface choice's
  // body is whatever package implements it, so the name alone proves nothing),
  // and every create must be a registry Holding that names the agent and no
  // party outside the agent, the registrar and the trusted registry parties.
  // A holding created for anyone else — the "redirected recipient" a
  // compromised relay would try — fails here; an undecodable create fails too.
  if (decoded.exercises.some(e => e.choiceId === REGISTRY_UTILITY_TWO_STEP_ACCEPT.choiceId)) {
    if (root.templateQualifiedName !== REGISTRY_TRANSFER_OFFER_TEMPLATE) {
      throw new PreparedTransferMismatchError(
        `registry accept is rooted on ${JSON.stringify(root.templateQualifiedName)} — refusing to sign ` +
          `(a registry two-step accept must be exercised on ${REGISTRY_TRANSFER_OFFER_TEMPLATE})`,
      );
    }
    const allowed = new Set<string>([
      expect.selfParty,
      expect.instrumentAdmin!,
      ...(expect.trustedRegistryParties ?? []),
    ]);
    for (const node of decoded.nodes) {
      if (node.create === undefined) continue;
      if (node.create.templateQualifiedName !== REGISTRY_HOLDING_TEMPLATE) {
        throw new PreparedTransferMismatchError(
          `registry accept creates ${JSON.stringify(node.create.templateQualifiedName)} — refusing to sign ` +
            `(an accept may only create the agent's ${REGISTRY_HOLDING_TEMPLATE})`,
        );
      }
      if (node.create.argument === undefined) {
        throw new PreparedTransferMismatchError(
          "registry accept creates a holding whose argument could not be decoded — refusing to sign",
        );
      }
      const leaves: string[] = [];
      collectPartyLeaves(node.create.argument, leaves, 0);
      const foreign = leaves.filter(p => !allowed.has(p));
      if (foreign.length > 0) {
        throw new PreparedTransferMismatchError(
          `registry accept creates a holding that names ${JSON.stringify([...new Set(foreign)])} — ` +
            `refusing to sign (a created holding may name only the agent, the registrar and the ` +
            `trusted registry parties; anything else is a redirected recipient)`,
        );
      }
      if (!leaves.includes(expect.selfParty)) {
        throw new PreparedTransferMismatchError(
          "registry accept creates a holding that does not name the agent — refusing to sign " +
            "(the accepted value must land on the agent)",
        );
      }
    }
  }
...
/* ════════════════════════════════════════════════════════════════════════
 * registry self-preapproval — verify-before-sign for the CREATE of a
 * `Utility.Registry.App.V0.Model.TransferPreapproval` (the non-Amulet, DA
 * Registry Utility analogue of the Amulet AmuletRules_CreateTransferPreapproval
 * self-provision). A self-custody wallet signs this with its OWN key to become
 * able to RECEIVE a registry token (USDCx, …) one-shot. THREAT MODEL: the relay
 * is MALICIOUS — it builds the prepared transaction, so the wallet must never
 * blind-sign it.
 *
 * The preapproval is a PLAIN CREATE (not a choice), whose receiver is the SOLE
 * SIGNATORY — so the ledger itself rejects a create whose receiver ≠ the signing
 * wallet (the wallet's signature cannot authorize a contract it is not a
 * signatory of). We additionally pin, from the bytes:
 *   - exactly ONE root node, a CREATE of the registry preapproval template, with
 *     NO exercises and no orphan/second-leg node (nothing else runs under the
 *     signature), and
 *   - the all-nodes foreign-party backstop: the only parties anywhere are the
 *     wallet itself, the instrument admin (registrar), and the OUT-OF-BAND-trusted
 *     registry infra parties (operator / bridge). A relay-injected foreign party
 *     — e.g. an attacker as receiver/operator — is refused here; anything that
 *     slipped past would still be rejected by the ledger's receiver-signatory
 *     rule. No merchant fee is charged (unlike Amulet), so a mis-built create
 *     costs nothing and moves nothing: denial, never loss.
 * ════════════════════════════════════════════════════════════════════════ */

/** `module:entity` of the DA Registry Utility preapproval template. Version-
 *  independent (package id omitted, like every other template pin here), so it
 *  matches across registry-app upgrades and every token in the registry family. */
const REGISTRY_PREAPPROVAL_TEMPLATE =
  "Utility.Registry.App.V0.Model.TransferPreapproval:TransferPreapproval";

export interface RegistrySelfPreapprovalExpectation {
  /** The self-custody wallet: the create's receiver (sole signatory) and act_as. */
  party: string;
  /** The instrument admin (registrar) of the token this preapproval is for. */
  admin: string;
  /** OUT-OF-BAND-trusted registry infra parties (operator / bridge) — the same
   *  set the transfer backstop uses. The create legitimately names the operator;
   *  admitting these (and only these, plus the wallet + admin) is the whole
   *  loss barrier. Sourced from config/constants, NEVER the relay. */
  trustedRegistryParties?: ReadonlySet<string>;
}

/** Assert the relay-prepared transaction is EXACTLY a self-create of this
 *  wallet's registry TransferPreapproval. Throws PreparedTransferMismatchError
 *
 * @param preparedTransactionB64
 * @param expect
 *  - otherwise. Call BEFORE signing. */
export function assertPreparedRegistrySelfPreapproval(
  preparedTransactionB64: string,
  expect: RegistrySelfPreapprovalExpectation,
): void {
  const decoded = decodePrepared(preparedTransactionB64);

  // (1) No exercises at all — a self-preapproval only CREATES.
  if (decoded.exercises.length > 0) {
    throw new PreparedTransferMismatchError(
      `registry self-preapproval: prepared transaction contains ${decoded.exercises.length} ` +
        `exercise(s) — refusing to sign (a self-preapproval only creates a contract)`,
    );
  }
  // (2) Every node recognized (no opaque node could run under our signature).
  for (const node of decoded.nodes) {
    if (!node.recognizedVersion || node.kind === undefined) {
      throw new PreparedTransferMismatchError(
        `registry self-preapproval: prepared transaction contains an unrecognized/opaque ` +
          `node (id ${JSON.stringify(node.nodeId)}) — refusing to sign`,
      );
    }
  }
  // (3) Exactly one root, and it is a CREATE of the registry preapproval template.
  if (decoded.roots.length !== 1) {
    throw new PreparedTransferMismatchError(
      `registry self-preapproval: prepared transaction has ${decoded.roots.length} root nodes ` +
        `(want exactly one create) — refusing to sign`,
    );
  }
  const byId = new Map<string, DecodedNode>();
  for (const node of decoded.nodes) {
    if (byId.has(node.nodeId)) {
      throw new PreparedTransferMismatchError(
        `registry self-preapproval: duplicate node id ${JSON.stringify(node.nodeId)} — refusing to sign`,
      );
    }
    byId.set(node.nodeId, node);
  }
  const rootId = decoded.roots[0] as string;
  const rootNode = byId.get(rootId);
  if (!rootNode || rootNode.kind !== "create") {
    throw new PreparedTransferMismatchError(
      `registry self-preapproval: root node is not a create (got ${JSON.stringify(rootNode?.kind ?? "unknown")}) ` +
        `— refusing to sign`,
    );
  }
  const tmpl = rootNode.create?.templateQualifiedName;
  if (tmpl !== REGISTRY_PREAPPROVAL_TEMPLATE) {
    throw new PreparedTransferMismatchError(
      `registry self-preapproval: root create is ${JSON.stringify(tmpl ?? null)}, not the registry ` +
        `TransferPreapproval template — refusing to sign`,
    );
  }
  // (4) No orphan/second-leg node: every node reachable from the single root.
  const reachable = new Set<string>();
  const stack = [rootId];
  while (stack.length > 0) {
    const id = stack.pop() as string;
  ...
  const orphans = decoded.nodes.filter(n => !reachable.has(n.nodeId));
  if (orphans.length > 0) {
    throw new PreparedTransferMismatchError(
      `registry self-preapproval: ${orphans.length} node(s) not reachable from the root create ` +
        `— refusing to sign (possible injected extra leg)`,
    );
  }

  // (5) act_as is exactly the wallet (the create is submitted under our authority).
  assertActAsIsSender(decoded.actAs, expect.party);

  // (5b) Pin the create's `receiver` field == the wallet, read directly from the
  // create argument (decl order: operator, receiver, instrumentAdmin,
  // instrumentAllowances). Defense-in-depth: the ledger already rejects a create
  // whose sole signatory (receiver) is not the signer, and the backstop already
  // refuses any foreign party — but a create naming a TRUSTED party (the operator)
  // as receiver would pass the backstop and only be caught at the ledger. Pinning
  // it here refuses to sign that up front. Best-effort: an argument shape we cannot
  // decode falls through to the ledger + backstop guarantees rather than throwing
  // on an honest tx whose encoding we did not anticipate.
  try {
    const arg = rootNode.create?.argument;
    if (arg !== undefined) {
      const entries = recordEntries(arg);
      // Position 1, not 2. MEASURED against the real MainNet fixture this file
      // is tested with: [0] operator, [1] receiver, [2] instrumentAdmin,
      // [3] instrumentAllowances — exactly the order the comment above states.
      //
      // Reading 2 did not merely check the wrong field, it disabled the check:
      // entryByDeclOrder fails closed when the label at that position is not
      // "receiver", and the PreparedDecodeError it throws is a SIBLING of
      // PreparedTransferMismatchError, so the catch below swallowed it and the
      // pin never executed. A guard that silently does nothing is worse than no
      // guard, because the comment above promises it as defense-in-depth.
      const receiverE = entryByDeclOrder(entries, 1, "receiver");
      const receiver = receiverE ? leafOf(receiverE.value) : undefined;
      if (receiver && receiver.kind === "party" && receiver.value !== expect.party) {
        throw new PreparedTransferMismatchError(
          `registry self-preapproval: create receiver is ${JSON.stringify(receiver.value)}, not ` +
            `this wallet ${JSON.stringify(expect.party)} — refusing to sign`,
        );
      }
    }
  } catch (e) {
    if (e instanceof PreparedTransferMismatchError) throw e;
    // decode-shape surprise → rely on the ledger signatory rule + backstop below.
  }

  // (6) All-nodes foreign-party backstop: the only parties anywhere are the wallet
  // (receiver/signatory), the instrument admin (registrar), and the out-of-band
  // trusted registry infra (operator/bridge). The create argument's parties land
  // in the `nodeValues` bucket (tagged by the create template), so the admin is
  // admitted as a trusted admin and the infra via trustedRegistryParties.
  const leaves = collectSplitPartyLeaves(decoded, rootId);
  assertNoForeignParties(
    leaves,
    new Set([expect.party]),
    expect.admin,
    0 /* a create root has no rootArg admin position */,
    true /* admin is caller-intent, independently trusted */,
    undefined /* no Amulet preapproval-provider role here */,
    expect.trustedRegistryParties ?? new Set(),
  );
}
